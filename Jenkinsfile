pipeline {
	...
	stage('TF Plan') {
              ...
              sh 'terraform plan  -out=myplan -input=true -var AWS_KEY_PAIR=${params.AWS_KEY_PAIR} -var AWS_SECURITY_GROUP=${params.AWS_SECURITY_GROUP} -var AWS_REGION=${params.AWS_REGION} '
       }
     }
   	stage('TF Apply') {
      	    steps {
